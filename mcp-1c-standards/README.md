...
Каталог с контентом в проекте: **`content/`**

- `content/doc-comment.md` — правила и примеры документирующего комментария (v8std 453, EDT).
- `content/lookup.json` — разделы стандартов (453, 641, doc-comment, naming, formatting) и коды диагностик BSL LS.
- **`content/std-*.md`** — полные тексты стандартов (скачаны с v8std.ru). Сейчас **24 стандарта** с полным текстом, в том числе:
...
Без аргументов скачиваются все ID из списка; можно передать отдельные номера: `python3 scripts/fetch_standards.py 453 641 437`. На v8std.ru есть не все стандарты (часть отдаёт 404). После загрузки скопируйте новые `std-*.md` и обновлённый `lookup.json` в `internal/tools/content/` и пересоберите бинарник, либо используйте каталог `content/` — сервис подхватит файлы при следующем запуске.
...
2. Каталог **`content/`** рядом с исполняемым файлом (например `mcp-1c-standards/content/`).
3. Каталог **`content/`** в текущей рабочей директории.
4. Если файлов нет — используется встроенный в бинарник контент (fallback).

Редактируйте `content/doc-comment.md` и `content/lookup.json` в проекте — после следующего вызова инструментов сервис подхватит изменения.
