...
- получение полного текста стандарта по ID (если он есть в `content/std-<id>.md`);
...
2. **Каталог `content/` рядом с исполняемым файлом** — например `mcp-1c-standards/content/` в той же папке, что и бинарник.
3. **Каталог `content/` в текущей рабочей директории** — удобно при запуске из корня проекта.
4. **Встроенный контент (embed)** — если ни один из каталогов не найден или в них нет нужного файла, используется контент, вшитый в бинарник при сборке (из `internal/tools/content/`).
