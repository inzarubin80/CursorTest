...
- **Явная загрузка:** инструмент **structure_load_rag_zip** — параметр `zipPath`; инструмент **structure_load_structure_xml** — параметр `xmlPath` (файл «Структура базы данных» в XML). Можно вызывать для смены источника без перезапуска сервера.
...
### 5. structure_load_structure_xml
